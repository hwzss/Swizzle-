...
BOOL classInstanceMethodSwizzle(Class aClass, SEL originalSel, SEL swizzleSel){
    ...
    //***
//    method_exchangeImplementations(originalMethod, swizzleMethod);//如果就写这一句，当secontroller未实现originalMethod方法时，会从父类找方法进行实现，所以就会替换掉UIViewController的方法。然后你又在WZ_viewWillApper调用了 [self WZ_viewWillApper:animated];。当父类的其他子类调用父类的viewWillApper时就会出现找不到WZ_viewWillApper错误❌。比如这里如果再替换方法前将grandParent以及parent类里的viewWillApper都注释掉，就会导致class_getInstanceMethod找到UIViewController类里的viewWillApper。这样当你运行时就会发现它的子类UINavigationController找不到WZ_viewWillApper而崩溃了。 -[UINavigationController WZ_viewWillApper:]: unrecognized selector sent to instance 0x11081fe00 2017-06-06 10:43:15.311105 Swizzle方法实现[41033:14732394] *** Terminating app due to uncaught exception 'NSInvalidArgumentException', reason: '-[UINavigationController WZ_viewWillApper:]: unrecognized selector sent to instance 0x11081fe00'
    //***
...
-(void)WZ_viewWillApper:(BOOL )animated{
    NSLog(@"哈哈哈.我是WZ_viewWillApper的IMP。我被调用了哈哈哈");
